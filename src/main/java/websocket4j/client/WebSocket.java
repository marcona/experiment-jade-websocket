...
public class WebSocket extends AbstractWebSocket {
     ...
     * @param timeout    number of milliseconds that the opening handshake can take before being cancelled (if 0 then it
     *                   will block forever)
     *
     * @throws IOException thrown when there is a problem with connection or protocol error
     */
    ...
    /**
     * Creates new web socket connected to given host and port, requesting given URI. This will block untill there is
    ...
    /**
     * Performs an opening handshake with the server. If this method completes successfully, a connection with the
     * server is established.
     *
     * @param arguments this should be String host, Integer port, String requestUri
     *
     * @throws IOException              thrown when there is a problem in socket communication or server does not follow
     *                                  the handshake protocol
     * @throws NoSuchAlgorithmException thrown when JVM doesn't support MD5 algorithm
     */
    @Override
    protected void handshake(Object... arguments) throws IOException,
                                                         NoSuchAlgorithmException {
        String host = (String)arguments[0];
        Integer port = (Integer)arguments[1];
        String requestUri = (String)arguments[2];
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            char finalString[] = new char[spaces + keyString.length() + garbage.length];
        ...
        while ((line = readLine()).length() != 0) {
            ...
            else if ("sec-websocket-origin".equals(name)) {
                if (!value.equals("http://" + host)) {
                    throw new IOException("Wrong value of sec-websocket-origin field: " + line);
                }
                origin = true;
            }
            else if ("sec-websocket-location".equals(name)) {
                if (!value.equals("ws://" + host + ":" + port + requestUri)) {
                    throw new IOException("Wrong value of sec-websocket-location field: " + line);
                }
                location = true;
            }
            else {
        ...
        byte[] validChallengeResponse = makeResponseToken(keys[0], keys[1], token);
        byte[] challengeResponse = readBytes(validChallengeResponse.length);
        for (int i = 0; i < validChallengeResponse.length; ++i) {
