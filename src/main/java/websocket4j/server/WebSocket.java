...
public class WebSocket extends AbstractWebSocket {
    ...
    /**
     * Creates new web socket ready for use, if opening handshake doesn't finish in given time, socket creation will be
     * aborted (if given time is 0, it will wait as long as needed).
     *
     * @param socket  socket to use for communication
     * @param timeout maximum time for handshake in milliseconds
     *
     * @throws IOException thrown when there is a problem with connection or protocol error
     */
    ...
    /**
     * Returns Request-URI part of the handshake header as specified in HTTP protocol. This will be null if the
     * handshake failed before Request-URI was sent.
     *
     ...
     */
    @Override
    protected void handshake(Object... arguments) throws IOException, NoSuchAlgorithmException {
        ...
        while ((line = readLine()).length() != 0) {
            ...
            else if (("sec-websocket-key1".equals(name))
                     || ("sec-websocket-key2".equals(name))) {
                Integer spaces = 0;
                Long number = (long)0;
