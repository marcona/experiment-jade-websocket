...
public abstract class AbstractWebSocket {
    ...
    /**
     * Creates new WebSocket instance using given socket for communication and limiting handshake time to given
     * milliseconds.
     *
     ...
     *
     * @throws IOException exception thrown when there is communication error or protocol error
     */
    protected AbstractWebSocket(Socket socket, Integer timeout, Object... args)
          throws IOException {
        this.socket = socket;
        ...
        if (taskBody.getException() != null) {
            socket.close();
            throw createIOException(taskBody.getException());
        }
    }
    ...
    protected String byteCollectionToString(Collection<Byte> collection) {
        ...
        int idx = 0;
        for (Byte aCollection : collection) {
            byteArray[idx++] = aCollection;
        }
     ...
     *
     * Close is brutal, without the closing handshake. It worked better in some web browsers, but it might change
     * sometime.
     *
     ...
     *
     * @throws IOException thrown when there is a problem with the connection or protocol
     */
    ...
    /**
     * Performs an opening handshake. If this method completes successfully, a connection with the other party is
     * established.
     *
     * @throws IOException              thrown when there is a problem in socket communication or client does not follow
     *                                  the handshake protocol
     * @throws NoSuchAlgorithmException thrown when JVM doesn't support MD5 algorithm
     */
    protected abstract void handshake(Object... arguments) throws IOException, NoSuchAlgorithmException;
     ...
     *
     * @param count number of sockets to read
     *
     * @return bytes that were read
     *
     * @throws IOException thrown when an error occurs while reading
     */
     ...
     *
     * @param key1  value of Sec-WebSocket-Key1 field
     * @param key2  value of Sec-WebSocket-Key2 field
     * @param token bytes sent after the fields
     *
     * @return response to this challenge
     *
     * @throws NoSuchAlgorithmException thrown if JVM doesn't support MD5 algorithm
     */
    ...
    private static IOException createIOException(Exception e) {
        IOException exception = new IOException("Handshake failed");
